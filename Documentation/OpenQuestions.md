...
5. **Post-Phase-20 selection:** owner reviews the gentle Journey Guidance experience and explicitly selects or defers any future bounded slice. Do not begin Phase 21 automatically.

## Resolved

- Engine: Unity **6000.5.4f1**, owner-approved and pinned for Phases 0–13.
- Pipeline: URP.
...
- Phase 11: after the Wayroot is restored, the three existing resource nodes use persisted UTC renewal deadlines and a fixed 20-second prototype interval. Legacy depleted IDs migrate to deadlines; no timer/notification/reward system was added.
- Phase 12: a built shelter can be rested at to persist one active return point; player defeat restores health and returns there, with RESET clearing it.
- Phase 13: restoring the first Wayroot visibly opens one compact Sunmeadow grove edge with one Thorn Guardian (8 health, 2 contact damage, 15-second fixed respawn). It uses existing attack, player health, shelter return, and Slime Core rewards only.
...
- Phase 20: the compact Journey card derives exactly one non-blocking recommended existing target from the persistent save: fresh meadow resource/merchant path, shelter, Wayroot, Thorn Guardian/grove, or Moonlit Bloomwell. Its warm firefly only appears for an off-screen target; after Bloomwell restoration it is a non-directive free-exploration state with no pointer. It adds no quest system, map, dialogue, auto-navigation, reward/item, unlock/requirement change, or Phase 21 work.
- Phase 32: reduced-flash and reduced-motion are compact PlayerPrefs-backed visual-comfort settings, intentionally separate from the versioned progression save schema. Both default OFF, persist through restart, remain independent of SOUND, and RESET restores OFF. Required gameplay feedback remains visible; no input, gameplay, route, or save-schema behavior changed.
