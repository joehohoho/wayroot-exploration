...
namespace Wayroot.Combat
{
    ...
    public sealed class EnemyEncounterPresentation : MonoBehaviour
    {
        ...
        public void Configure(PrototypeEnemy enemy, PrototypeEnemyChase chase, bool guardian)
        {
            _enemy = enemy;
            _chase = chase;
            _guardian = guardian;
        ...
        private void Update()
        {
            ...
            bool hit = CombatEncounterPolishRules.IsActive(_enemy.HitElapsed, CombatEncounterPolishRules.EnemyHitFlashSeconds) && !_enemy.IsDefeated;
            ...
            if (hit) _hitFlash.localScale = (_guardian ? Vector3.one * 1.62f : Vector3.one * 1.38f) * (1f + 0.18f * Mathf.Sin(time * 28f));
